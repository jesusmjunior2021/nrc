...
def criar_graficos_avancados(df: pd.DataFrame):
    ...
    with col2:
        ...
        if opcoes_agrupamento:
            ...
            agrupamento = None
            st.info("Sem colunas de agrupamento disponíveis nos dados carregados.")
    ...
    elif tipo_analise == "Motivos de Não Conformidade":
        criar_grafico_motivos(df)

    elif agrupamento is None:
        st.warning("Esta análise precisa de uma coluna de agrupamento (município, serventia, ano ou mês), que não está disponível nos dados carregados.")

    elif tipo_analise == "Nascimentos vs Registros":
...
def criar_grafico_motivos(df: pd.DataFrame):
    """Analisa a coluna de texto livre com os motivos de não conformidade
    informados pelas serventias (dado da planilha nunca antes exibido)."""
    st.subheader("📝 Motivos de Não Conformidade Mais Informados")

    if 'motivos' not in df.columns:
        st.info("Coluna de motivos não disponível nos dados carregados.")
        return

    respostas = df['motivos'].dropna().astype(str).str.strip()
    respostas = respostas[respostas != '']
    respostas = respostas[~respostas.str.lower().isin([
        'nan', 'none', 'n/a', 'na', 'não informado', 'nao informado', '-', '.'
    ])]

    total_com_motivo = len(respostas)
    total_registros = len(df)
    st.metric(
        "📋 Registros com motivo informado",
        f"{total_com_motivo:,}",
        f"{(total_com_motivo/total_registros*100):.1f}% do total" if total_registros else None
    )

    if total_com_motivo == 0:
        st.info("Nenhum motivo de não conformidade foi informado no recorte atual.")
        return

    st.markdown("**Respostas mais recorrentes (texto exato informado pela serventia):**")
    contagem = respostas.value_counts().head(15)
    st.bar_chart(contagem)

    with st.expander("📄 Ver todas as respostas do recorte atual"):
        st.dataframe(respostas.value_counts().reset_index().rename(
            columns={'index': 'Motivo informado', 'motivos': 'Motivo informado', 'count': 'Ocorrências'}
...
def main():
    ...
    with tab1:
        ...
        with col1:
            ...
            if 'percentual' in df_filtrado.columns and 'municipio' in df_filtrado.columns and not df_filtrado.empty:
                media_por_municipio = df_filtrado.groupby('municipio')['percentual'].mean()
                if not media_por_municipio.empty:
                    melhor_municipio = media_por_municipio.idxmax()
                    melhor_perc = media_por_municipio.max()
                    st.success(f"🏆 Melhor: **{melhor_municipio}** ({melhor_perc:.1f}%)")
                    
                    pior_municipio = media_por_municipio.idxmin()
                    pior_perc = media_por_municipio.min()
                    st.error(f"⚠️ Precisa atenção: **{pior_municipio}** ({pior_perc:.1f}%)")
                else:
                    st.info("Sem dados suficientes para calcular ranking de municípios.")
            else:
                st.info("Sem dados suficientes para calcular ranking de municípios.")
...
if __name__ == "__main__":
    try:
        ...
        st.error("⚠️ Ocorreu um erro ao renderizar o dashboard.")
        with st.expander("🔍 Detalhes técnicos do erro (clique para expandir)", expanded=True):
            st.exception(e)
        st.info(
            "Dica: se o erro citar uma coluna ausente (ex.: 'municipio'), a fonte de "
            "dados carregada pode ter um schema diferente do esperado (ex.: outra aba "
            "da planilha). Confira a URL/arquivo carregado na barra lateral."
        )
